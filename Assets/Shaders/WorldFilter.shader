Shader "Custom/WorldFilter"
{
	Properties
	{
		_MainTex ("Texture", 2D) = "white" {}
		_FilterNormal("FilterNormal",Vector) = (1,0,0,0)
		_FilterPosition("FilterPosition",Vector) = (1,0,0,0)
	}
	SubShader
	{
		Tags { "RenderType"="Opaque" }
		LOD 100

		Pass
		{
			CGPROGRAM
			#pragma vertex vert
			#pragma fragment frag
			// make fog work
			#pragma multi_compile_fog
			
			#include "UnityCG.cginc"

			struct appdata
			{
				float4 vertex : POSITION;
				float2 uv : TEXCOORD0;
				float3 worldPos : TEXCOORD1;
			};

			struct v2f
			{
				float2 uv : TEXCOORD0;
				UNITY_FOG_COORDS(1)
				float4 vertex : SV_POSITION;
				float3 worldPos : TEXCOORD1;
			};

			sampler2D _MainTex;
			float4 _MainTex_ST;

			float3 _FilterNormal;
			float3 _FilterPosition;


			bool checkVisability(fixed3 worldPos)
			{
				float dotProd1 = dot(worldPos - _FilterPosition, _FilterNormal);
				return dotProd1 > 0;
			}

			v2f vert (appdata v)
			{
				v2f o;
				o.vertex = UnityObjectToClipPos(v.vertex);
				o.uv = TRANSFORM_TEX(v.uv, _MainTex);
				UNITY_TRANSFER_FOG(o,o.vertex);

				o.worldPos = mul(unity_ObjectToWorld, v.vertex);

				return o;
			}
			
			fixed4 frag (v2f i) : SV_Target
			{
				if (checkVisability(i.worldPos))discard;
				// sample the texture
				fixed4 col = tex2D(_MainTex, i.uv) + step(0, dot(i.worldPos - (_FilterPosition - _FilterNormal * 0.001), _FilterNormal)) * fixed4(1, 1, 1, 1);
				// apply fog
				UNITY_APPLY_FOG(i.fogCoord, col);
				return col;
			}
			ENDCG
		}
	}
}
